...
2. 在 Obsidian 中打开 `test-vault` 文件夹作为仓库
3. 在设置 → 社区插件中启用插件
4. 修改源代码后会自动重载
...
2. 将以下文件复制到你的仓库的 `.obsidian/plugins/vault-cleaner/` 目录：
   - `main.js`
   - `manifest.json`
   - `styles.css`
3. 在设置 → 社区插件中启用插件
4. 运行 `npm run dev` 自动重新构建，然后重新加载 Obsidian 查看更改

#### 方法三：使用 hot-reload 插件

1. 在你的仓库中安装 [hot-reload](https://github.com/pjeby/hot-reload) 插件
2. 在仓库根目录创建名为 `.hot-reload` 的文件
3. 运行 `npm run dev` - 更改会自动重载，无需重启 Obsidian

### 发布

```bash
# 版本号更新（更新 package.json、manifest.json）
npm run version
...
在 GitHub 上创建新 Release，上传构建文件（`main.js`、`manifest.json`、`styles.css`）。
