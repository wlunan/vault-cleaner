...
一个文件被认为是**孤立文件**，当它同时满足以下所有条件：

- **无入站链接**：没有其他文件链接到它
- **无出站链接**：它没有链接到任何其他文件（包括链接到附件也不算孤立）
- **无 Canvas 引用**：没有在任何 Canvas 文件中被引用

### 附件检测

当文件匹配您配置的附件路径时，会被分类为**附件**：

| 路径类型 | 示例 | 匹配逻辑 |
|----------|------|----------|
| 绝对路径 | `attachments` | 文件父路径或文件路径以它开头 |
| 相对路径 | `./attachments` | 文件路径以 `attachments` 开头（默认）或任何父目录名称匹配 `attachments`（替代算法） |

### 排除扫描的文件

- 白名单文件夹中的文件
- 匹配忽略模式的文件（正则表达式）
- 最近修改的文件（在保护期内，默认 7 天）
- `.obsidian/` 和其他系统文件夹中的文件
